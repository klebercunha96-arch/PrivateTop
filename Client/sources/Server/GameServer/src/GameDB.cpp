...
// Declarações externas para funções utilitárias de SQL
extern short exec_sql_direct(const char* sql);
extern int _get_row(std::string* buf, int n, const char* param, const char* query);

char	szDBLog[256] = "DBData";

// ================== Mercado Global ==================
// Insere um item no mercado global
bool CGameDB::InsertMarketItem(int item_id, int quantidade, int preco, int vendedor_id) {
    char query[512];
    sprintf(query,
        "INSERT INTO MarketGlobal (item_id, quantidade, preco, vendedor_id, status, data_listagem) "
        "VALUES (%d, %d, %d, %d, 'a_venda', GETDATE())",
        item_id, quantidade, preco, vendedor_id);

    SQLRETURN sExec = _tab_log->exec_sql_direct(query);
    return DBOK(sExec);
}

// Marca item como vendido e registra o comprador
bool CGameDB::ComprarItemMercado(int market_id, int comprador_id) {
    char query[512];
    sprintf(query,
        "UPDATE MarketGlobal SET status = 'vendido', comprador_id = %d, data_venda = GETDATE() "
        "WHERE market_id = %d AND status = 'a_venda'",
        comprador_id, market_id);

    SQLRETURN sExec = _tab_log->exec_sql_direct(query);
    return DBOK(sExec);
}

// Resgata o dinheiro das vendas já concluídas
enum { MAX_SQL_RESULT = 64 };
int CGameDB::ResgatarDinheiroVendas(int vendedor_id) {
	char query[256];
	sprintf(query, "SELECT SUM(preco) FROM MarketGlobal WHERE vendedor_id = %d AND status = 'vendido'", vendedor_id);
	string buf[MAX_SQL_RESULT];
	char param[] = "SUM(preco)";
	int r = _tab_log->_get_row(buf, MAX_SQL_RESULT, param, query);
	int total = 0;
	if (DBOK(r) && buf[0] != "") {
		total = atoi(buf[0].c_str());
	}
	sprintf(query, "UPDATE MarketGlobal SET status = 'resgatado' WHERE vendedor_id = %d AND status = 'vendido'", vendedor_id);
	_tab_log->exec_sql_direct(query);
	return total;
}

// Busca itens à venda no mercado global
// TODO: Implemente usando o padrão do seu projeto para SELECT múltiplo
std::vector<MarketItem> CGameDB::BuscarItensMercado() {
	std::vector<MarketItem> itens;
	// Exemplo:
	// char query[256];
	// sprintf(query, "SELECT market_id, item_id, quantidade, preco, vendedor_id FROM MarketGlobal WHERE status = 'a_venda'");
	// ... preencher vetor itens usando _get_row ou equivalente ...
	return itens;
}
// ================== Fim Mercado Global ==================
