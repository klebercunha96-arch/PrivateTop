...
public:
	// -------------------- Mercado Global --------------------
	// Busca todos os itens disponíveis no mercado global
	std::vector<MarketItem> BuscarItensMercado();
	// Compra um item do mercado global
	bool ComprarItemMercado(int market_id, int comprador_id);
	// Resgata dinheiro das vendas realizadas
	int ResgatarDinheiroVendas(int vendedor_id);
	// Insere um item no mercado global
	bool InsertMarketItem(int item_id, int quantidade, int preco, int vendedor_id);
	// ...existing code...
