void CS_MarketList() {
	LPPACKET pk = CreatePacket(CMD_CS_MARKET_LIST);
	SendPacket(pk);
}

void CS_MarketAddItem(int item_id, int quantidade, int preco) {
	LPPACKET pk = CreatePacket(CMD_CS_MARKET_ADD_ITEM);
	pk->WriteInt(item_id);
	pk->WriteInt(quantidade);
	pk->WriteInt(preco);
	SendPacket(pk);
}

void CS_MarketBuy(int market_id) {
	LPPACKET pk = CreatePacket(CMD_CS_MARKET_BUY);
	pk->WriteInt(market_id);
	SendPacket(pk);
}

void CS_MarketFetchMoney() {
	LPPACKET pk = CreatePacket(CMD_CS_MARKET_FETCH_MONEY);
	SendPacket(pk);
}
#include "StdAfx.h"
#include "PacketCmd.h"
