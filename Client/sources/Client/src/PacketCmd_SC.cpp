BOOL SC_MarketList(LPRPACKET pk) {
	// Limpa a lista antes de adicionar novos itens
	MarketUI_ClearItems();
	int itemCount = pk->ReadInt();
	for (int i = 0; i < itemCount; ++i) {
		int market_id = pk->ReadInt();
		int item_id = pk->ReadInt();
		int quantidade = pk->ReadInt();
		int preco = pk->ReadInt();
		int vendedor_id = pk->ReadInt();
		MarketUI_AddItem(market_id, item_id, quantidade, preco, vendedor_id);
	}
	MarketUI_Refresh();
	return TRUE;
}

// Handler de resposta da compra
BOOL SC_MarketBuyResult(LPRPACKET pk) {
	int result = pk->ReadInt();
	int market_id = pk->ReadInt();
	// result: 1 = sucesso, 0 = erro
	MarketUI_OnBuyResult(result, market_id);
	return TRUE;
}

// Handler de resposta da venda
BOOL SC_MarketAddItemResult(LPRPACKET pk) {
	int result = pk->ReadInt();
	int item_id = pk->ReadInt();
	MarketUI_OnAddItemResult(result, item_id);
	return TRUE;
}

// Handler de resposta do resgate
BOOL SC_MarketFetchMoneyResult(LPRPACKET pk) {
	int result = pk->ReadInt();
	int valor = pk->ReadInt();
	MarketUI_OnFetchMoneyResult(result, valor);
	return TRUE;
}
