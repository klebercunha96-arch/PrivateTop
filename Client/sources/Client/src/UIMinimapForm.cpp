...
// Mercado Global - método da CMiniMapMgr
void CMiniMapMgr::OpenMarketGlobal()
{
	CForm* frmMarketGlobal = _FindForm("frmMarketGlobal");
	if (frmMarketGlobal)
	{
		LG("market", "Opening Global Market form\n");
		frmMarketGlobal->Show();
	}
	else
	{
		LG("market", "ERROR: frmMarketGlobal not found!\n");
		g_pGameApp->SysInfo("Global Market form not found");
	}
}
...
void CMiniMapMgr::_MiniFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	else if (name == "btnMarketGlobal")
	{
		g_stUIMap.OpenMarketGlobal();
	}
