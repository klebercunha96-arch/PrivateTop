...
-- Exemplo de comandos client para Mercado Global
-- Enviar pacote para listar itens
function MarketGlobal_RequestList()
	local pkt = GetPacket()
	WriteCmd(pkt, 1001) -- CMD_CM_MARKET_LIST
	SendPacket(pkt)
end

-- Enviar pacote para adicionar item
function MarketGlobal_AddItem(id_item, preco, quantidade)
	local pkt = GetPacket()
	WriteCmd(pkt, 1002) -- CMD_CM_MARKET_ADD
	WriteDword(pkt, id_item)
	WriteDword(pkt, preco)
	WriteWord(pkt, quantidade)
	SendPacket(pkt)
end

-- Enviar pacote para comprar item
function MarketGlobal_BuyItem(id_mercado)
	local pkt = GetPacket()
	WriteCmd(pkt, 1003) -- CMD_CM_MARKET_BUY
	WriteDword(pkt, id_mercado)
	SendPacket(pkt)
end

-- Enviar pacote para remover item
function MarketGlobal_RemoveItem(id_mercado)
	local pkt = GetPacket()
	WriteCmd(pkt, 1004) -- CMD_CM_MARKET_REMOVE
	WriteDword(pkt, id_mercado)
	SendPacket(pkt)
end

-- =========================
-- Eventos dos bot?es do Mercado Global
-- =========================

function OnClick_btnAtualizar()
    if CS_MarketGlobal_UpdateList then
        CS_MarketGlobal_UpdateList()
    else
        print("Fun??o CS_MarketGlobal_UpdateList n?o encontrada!")
    end
end

function OnClick_btnCadastrar()
    if CS_MarketGlobal_RegisterItem then
        CS_MarketGlobal_RegisterItem()
    else
        print("Fun??o CS_MarketGlobal_RegisterItem n?o encontrada!")
    end
end

function OnClick_btnComprar()
    if CS_MarketGlobal_BuyItem then
        CS_MarketGlobal_BuyItem()
    else
        print("Fun??o CS_MarketGlobal_BuyItem n?o encontrada!")
    end
end

function OnClick_btnRemover()
    if CS_MarketGlobal_RemoveItem then
        CS_MarketGlobal_RemoveItem()
    else
        print("Fun??o CS_MarketGlobal_RemoveItem n?o encontrada!")
    end
end

function OnClick_btnFechar()
    UI_ShowForm(frmMarketGlobal, FALSE)
end

-- Associar eventos aos bot?es
--UI_SetButtonClickEvent(btnAtualizar, "OnClick_btnAtualizar")
--UI_SetButtonClickEvent(btnCadastrar, "OnClick_btnCadastrar")
--UI_SetButtonClickEvent(btnComprar, "OnClick_btnComprar")
--UI_SetButtonClickEvent(btnRemover, "OnClick_btnRemover")
--UI_SetButtonClickEvent(btnFechar, "OnClick_btnFechar")

-- =========================
-- Fim eventos Mercado Global
-- =========================