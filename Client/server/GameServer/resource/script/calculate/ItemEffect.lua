...
function Corsairs_WeaponChest2(role, Item)
    ...
    if Chest[ItemID] ~= nil then
        ...
        local randIndex = math.random(1, #Chest[ItemID].Item)
        local itemGiven = Chest[ItemID].Item[randIndex]
        GiveItem(role, 0, itemGiven, Chest[ItemID].Quantity, Chest[ItemID].Quality)
        SystemNotice(role, "Obtains "..GetItemName(itemGiven)..".")
    end
end
