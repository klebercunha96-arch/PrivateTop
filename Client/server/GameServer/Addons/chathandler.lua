...
cmd.list['getcrystals'] = {
    gm = 99,
    param = {"number"},
    func = function(role, param) GiveIMP(role, tonumber(param[1]) or 0) end
}
...
-- Mercado Global: comandos de chat
function Cmd_MarketAdd(role, param)
	local id_item = tonumber(param[1])
	local preco = tonumber(param[2])
	local quantidade = tonumber(param[3])
	if id_item and preco and quantidade then
		MarketGlobal_AddItem(role, {
			id_item = id_item,
			preco = preco,
			quantidade = quantidade
		})
		PopupNotice(role, "Item cadastrado no Mercado Global! Preço em dinheiro do inventário.")
	else
		PopupNotice(role, "Parâmetros inválidos. Use: /marketadd id_item,preco,quantidade (preço em dinheiro do inventário)")
	end
end

function Cmd_MarketList(role, param)
	local list = MarketGlobal_List()
	if #list == 0 then
		PopupNotice(role, "Nenhum item no Mercado Global.")
	else
		for _, item in ipairs(list) do
			PopupNotice(role, string.format("ID:%d Item:%d Preço:%d Qtd:%d Vendedor:%s", item.id_mercado, item.id_item, item.preco, item.quantidade or 1, item.vendedor or '?'))
		end
	end
end

function Cmd_MarketBuy(role, param)
	local id_mercado = tonumber(param[1])
	if id_mercado then
		local ok = MarketGlobal_BuyItem(role, id_mercado)
		if ok then
			PopupNotice(role, "Compra realizada com sucesso!")
		else
			PopupNotice(role, "Falha na compra: saldo insuficiente ou item indisponível.")
		end
	else
		PopupNotice(role, "Parâmetro inválido. Use: /marketbuy id_mercado")
	end
end

function Cmd_MarketRemove(role, param)
	local id_mercado = tonumber(param[1])
	if id_mercado then
		MarketGlobal_RemoveItem(id_mercado)
		PopupNotice(role, "Item removido do Mercado Global.")
	else
		PopupNotice(role, "Parâmetro inválido. Use: /marketremove id_mercado")
	end
end



function Cmd_MarketResgatar(role, param)
	MarketGlobal_ResgatarVendas(role)
end
...
cmd.list["marketadd"] = {func = Cmd_MarketAdd, gm = 1, param = {"number", "number", "number"}}
cmd.list["marketlist"] = {func = Cmd_MarketList, gm = 1, param = {}}
cmd.list["marketbuy"] = {func = Cmd_MarketBuy, gm = 1, param = {"number"}}
cmd.list["marketremove"] = {func = Cmd_MarketRemove, gm = 1, param = {"number"}}
cmd.list["marketresgatar"] = {func = Cmd_MarketResgatar, gm = 1, param = {}}